...
type Order = {
  ...
  total_amount: number; advance_amount: number; product_cost: number; delivery_charge: number; boost_cost: number;
  total_cost: number; profit: number; status: string; payment_status: string; tracking_code?: string;
};
...
function scoreFromFraudResponse(payload:any):number {
  ...
  const numericDirect=Number(direct); if(Number.isFinite(numericDirect)) return Math.max(0,Math.min(100,Math.round(numericDirect)));
...
export default function OrdersHistoryPage() {
  ...
  return <div className="p-6">
    ...
    <div className="mb-6 flex flex-wrap items-center gap-3">
      <select value={selectedMonth} onChange={e=>{setSelectedMonth(e.target.value);setSelectedDate("");}} className="rounded-lg border bg-white p-3 font-medium">
        <option value="all">All Months</option>
        {Array.from({length:12},(_,i)=>{const month=String(i+1).padStart(2,'0');return <option key={month} value={`2026-${month}`}>{new Date(2026,i,1).toLocaleString('en-US',{month:'long'})} 2026</option>;})}
      </select>
      <div className="flex items-center gap-2 rounded-lg border bg-white p-2">
        ...
        {selectedDate&&<button onClick={()=>setSelectedDate("")} className="px-1 text-xs font-bold text-red-500">✕</button>}
      </div>
    ...
    <div className="overflow-x-auto rounded-xl bg-white shadow">
      {searchQuery.trim()||selectedDate ? (
        ...
        <table className="w-full">
          ...
          <tbody>
            {dateList.length===0?<tr><td colSpan={5} className="p-6 text-center text-gray-500">No orders found.</td></tr>:dateList.map(date=>{
    ...
    {selectedDateOrders && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
        <div className="max-h-[90vh] w-full max-w-[95vw] overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
          ...
          <div className="mt-6 flex justify-end"><button onClick={()=>setSelectedDateOrders(null)} className="rounded-lg border bg-gray-100 px-5 py-2">Close</button></div>
        </div>
      </div>
    )}
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
        <div className="w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl">
          <div className="mb-4 flex items-center justify-between border-b pb-3">
            ...
            <button onClick={()=>setViewingOrder(null)} className="text-2xl text-gray-500">×</button>
          </div>
