...
export default function OrderHistoryPage() {
  ...
  async function loadAndAutoSyncOrders() {
    ...
    const fetchedOrders = (data || []).map((item) => {
      const productCost = Number(item.product_cost) || 0;
      const delivery = Number(item.delivery_charge) || 0;
      const boost = Number(item.boost_cost) || 0;
      const totalAmount = Number(item.total_amount) || 0;
      
      const calculatedTotalCost = productCost + delivery + boost;
      const calculatedProfit = totalAmount - calculatedTotalCost;
  ...
  async function updateOrder() {
    ...
    const rawUpdated = data as Order;
    const updatedItem: Order = {
      ...rawUpdated,
      total_cost: (Number(rawUpdated.product_cost) || 0) + (Number(rawUpdated.delivery_charge) || 0) + (Number(rawUpdated.boost_cost) || 0),
      profit: (Number(rawUpdated.total_amount) || 0) - ((Number(rawUpdated.product_cost) || 0) + (Number(rawUpdated.delivery_charge) || 0) + (Number(rawUpdated.boost_cost) || 0)),
    };
          ...
          <div className="w-full max-w-xl rounded-2xl bg-white p-6 shadow-2xl">
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between border-b pb-2">
                ...
                <span className="font-semibold text-green-600">৳{viewingOrder.profit}</span>
              </div>
